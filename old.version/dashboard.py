...
def load_spe_data():
    ...
    if os.path.exists(file_path):
        try:
            ...
            # Ejemplo: Convertir columnas a tipos específicos
            if 'Fecha' in data.columns:
                data['Fecha'] = pd.to_datetime(data['Fecha'], errors='coerce')
            return data
        except Exception as e:
            st.error(f"Error al leer el archivo MATRIZ.xlsx: {e}")
            return None
    else:
        st.warning("El archivo MATRIZ.xlsx no existe en la carpeta descargas/SPE.")
        return None
    ...
    try:
        ...
        if registros_historicos:
            ...
            if not df_historico.empty:
                ...
                df_historico.index += 1  # Ajusta el índice para que comience desde 1
