...
def render_pending_reports_tab(data: pd.DataFrame, module_name: str = None):
    """
    Renderiza el reporte de expedientes pendientes.
        ...
        try:
            ...
            if view_type == "Activos":
                filtered_data = filtered_data[
                    (~filtered_data['EVALASIGN'].isin(INACTIVE_EVALUATORS.get(module_name, []))) &
                    (~filtered_data['EVALASIGN'].isin(VULNERABILIDAD_EVALUATORS.get(module_name, []))) &
            ...
            def get_status(row):
                try:
                    if pd.isna(row['EVALASIGN']) or str(row['EVALASIGN']).strip() == '':
                        return 'No Asignado'
                    elif row['EVALASIGN'] in VULNERABILIDAD_EVALUATORS.get(module_name, []):
                        return 'Vulnerabilidad'
                    elif str(row['EVALASIGN']) == 'SUSPENDIDA':
                        return 'Suspendida'
                    elif row['EVALASIGN'] in INACTIVE_EVALUATORS.get(module_name, []):
                        return 'Inactivos'
                    else:
                        return 'Activos'
                except Exception as e:
